export function checkName(value: string) {
  ...
  if (value.length < 2) return "The name field is too short.";
...
export function checkEmail(value: string) {
  ...
  if (!value) return "You need an email.";
  if (value.match(/@/g)?.length !== 1) return 'You need just one "@".';
  const [root, domain] = value.split("@");

  //Checking the root
  if (root.match(regex)?.length !== root.length) return "You need an email.";
  if (root.startsWith(".") || root.endsWith("."))
    return 'You may not start or end with a dot before "@".';

  const regexDots = /\.{2,}/;
  if (regexDots.test(root)) return "You may not have sequencial dots.";

  //Checking domain
  if (domain.match(/[.]/g)?.length !== 1)
    return 'You need an domain after "@".';
  const [initial, com] = domain.split(".");

  if (!initial) return "You do not have a domain.";
  if (!com.endsWith("com")) return 'You need finish with ".com".';

  return null;
}

export function checkSubject(value: string) {
  if (value.length === 0) return "You need a subject.";
  if (value.length > 255) return "Too many caracteres in subject.";
  return null;
}
